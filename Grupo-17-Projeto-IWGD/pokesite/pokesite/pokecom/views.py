...
def fazer_upload(request):
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        pokemon_name = request.POST.get('nome_pokemon').title()
        raridade = request.POST.get('raridade')
        colecao = request.POST.get('colecao').title()
...
def troca_form(request):
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        preco = request.POST.get('preco')
        vend = request.user.username
        trade = request.POST.get('pref1').title()
...
def proc_rating(request):
    if request.method == 'POST':
        ...
        if classif.exists():
            ...
            return render(request, 'pokecom/proc_rating.html', {"rating":round(rating_sum/len(classif), 2), "comment":classif})
